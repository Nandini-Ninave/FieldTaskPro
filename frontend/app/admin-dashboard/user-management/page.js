  ...
  export default function UserManagement() {
    ...
    const resetForm = () => {
      setFormData({
        name: "", email: "", password: "", role: "member", organizationId: "", attendanceMode: "A", gpsEnabled: true, locationConsent: false, assignedZones: [], shiftStart: "09:00", shiftEnd: "18:00", isActive: true,
      });
    };
    return (
      ...
      <div className="userPage">
        <div className="formBox">
          ...
          <select name="organizationId" value={formData.organizationId} onChange={handleChange}>
            ...
            {
              organizations.map(
                (org) => (<option key={org._id} value={org._id}>{org.name}</option>
                )
              )
            }
          </select>
          ...
          <select
            name="attendanceMode" value={formData.attendanceMode} onChange={handleChange}>
            <option value="A">Mode A</option>
            <option value="B">Mode B</option>
          </select>
          {
            formData.attendanceMode === "A" && (
              <select multiple name="assignedZones" value={formData.assignedZones} onChange={handleZoneChange} className="zoneSelect">
                {
                  zones.filter((z) => {
                      const orgId =
                        typeof z.organizationId === "object" ? z.organizationId._id : z.organizationId;
                      return (String(orgId) === String(formData.organizationId))
                    })
                    .map((zone) => (
          ...
          <div className="checkRow">
            <label>
              <input type="checkbox" name="gpsEnabled" checked={formData.gpsEnabled} onChange={handleChange}/>
              GPS Enabled
            </label>
            <label>
              <input type="checkbox" name="locationConsent" checked={formData.locationConsent} onChange={handleChange}/>Location Consent
            </label>
            <label>
          ...
          {editId ? (<button onClick={updateUser}>Update User</button>) : ( <button onClick={addUser}>Add User</button>)}
        </div>
            ...
            <tbody>
              {
                users.map((user) => (
                  <tr key={user._id}>
                    ...
                    <td>{user.attendanceMode}</td>
                    <td>{user.assignedZones ?.map((z) => z.name).join(", ")}</td>
